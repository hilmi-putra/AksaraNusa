...
import { getAdminBooks, updateAdminBookIsbn } from "@/lib/api/books";
...
export default function AdminIsbnPage() {
  ...
  const [formData, setFormData] = useState({ isbn: "", edition: "" });
  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    setItems([{ label: "Manajemen Buku", href: "/admin/books" }, { label: "ISBN & Edisi" }]);
  }, [setItems]);
  ...
  const handleEditClick = (book: ApiBook) => {
    setEditingBook(book);
    setFormData({ isbn: book.isbn || "", edition: book.edition || "" });
    setIsEditOpen(true);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      await updateAdminBookIsbn(editingBook.id, formData);
      toast.success("ISBN & Edisi berhasil diperbarui");
      setIsEditOpen(false);
      fetchData();
    } catch (error: any) {
  ...
  const columns: ColumnDef<ApiBook>[] = [
    ...
    },
    {
      ...
      header: "ISBN",
      cell: ({ row }) => (
        <span className="text-sm font-mono text-gray-600">{row.original.isbn || "-"}</span>
      ),
    },
    {
      accessorKey: "edition",
      header: "Edisi / Cetakan",
      cell: ({ row }) => (
        <span className="text-sm text-gray-600">{row.original.edition || "-"}</span>
      ),
    },
    {
      id: "actions",
      ...
      cell: ({ row }) => (
        <Button variant="outline" size="sm" onClick={() => handleEditClick(row.original)}>
          <Edit className="w-4 h-4 mr-2" /> Update ISBN
        </Button>
      ),
    },
  ];
    ...
    <AdminDashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-900">ISBN & Edisi</h1>
            <p className="text-sm text-gray-500 mt-1">Kelola data ISBN dan cetakan buku</p>
          </div>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Update ISBN & Edisi</DialogTitle>
          </DialogHeader>
          {editingBook && (
            <div className="space-y-4 py-4">
              ...
              <div className="grid gap-2">
                <Label htmlFor="edition">Edisi / Cetakan</Label>
                <Input
                  id="edition"
